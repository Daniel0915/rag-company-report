...
근거로 답변합니다. 대화 히스토리를 참고해 후속 질문을 재작성하고, 로컬(Ollama)/클라우드
(Gemini) LLM을 선택해 답변 품질을 비교할 수 있습니다.

## 사용 화면

![답변 모델(로컬/클라우드) 토글과 검색 범위(topK) 조절](docs/screenshots/02-chat-controls.png)

기업을 선택하고 최신 공시를 색인한 뒤, 선택한 기업의 공시 내용에 대해 자유롭게
질문하면 답변과 함께 근거가 된 공시(회사명/보고서명/접수번호/섹션)를 보여줍니다.
채팅창 상단에서 답변 모델(로컬 qwen2.5:3b / 클라우드 Gemini)과 검색 범위(topK)를
...
flowchart LR
    ...
    CHAT -- "답변 생성 (로컬 또는 클라우드)" --> OLLAMA
    CHAT -. "provider=cloud" .-> GEMINI
```
...
지분공시 그래프에서 제출인 정보 보강 → context 기반 답변 생성. 로컬(Ollama qwen2.5:3b)과
클라우드(Gemini) 중 선택해서 같은 질문의 답변 품질을 비교할 수 있습니다.

## 기술 스택

- **Backend**: Spring Boot 3.4, Spring AI 1.0 (Ollama 모델 + Neo4j 벡터스토어)
- **LLM/임베딩**: Ollama (`qwen2.5:3b` 채팅, `bge-m3` 임베딩), 선택적으로 Google
  Gemini(REST API 직접 호출 — Spring AI Google GenAI 스타터는 1.1.0+ 필요해서
  현재 1.0.0 BOM과 맞지 않아 `RestClient`로 직접 연동)
...
**Gemini API 키 (선택)**: 채팅에서 "클라우드" 모델을 쓰려면
[Google AI Studio](https://aistudio.google.com/apikey)에서 발급받은 키(`AIzaSy...`
형태)를 `GEMINI_API_KEY` 환경변수로 설정합니다. 설정 안 하면 클라우드 옵션 선택 시
...
2. 채팅창 상단에서 답변 모델(로컬/클라우드)과 검색 범위(topK) 선택
...
뒤 검색합니다. `provider`는 `"local"`(기본값, Ollama) 또는 `"cloud"`(Gemini).
...
src/main/resources/
├── application.yml                      # Ollama / Neo4j / DART / Gemini 설정
...
└── src/
    ...
    └── components/
        ...
        ├── ChatPanel.vue                # 채팅 UI (로컬/클라우드 토글, topK, 출처, 히스토리)
...
- qwen2.5:3b는 3B급 소형 모델이라 후속 질문 재작성/답변 품질이 완벽하지 않을 수
  있습니다 — 이럴 때 클라우드(Gemini) 옵션으로 같은 질문을 비교해보는 용도입니다.
